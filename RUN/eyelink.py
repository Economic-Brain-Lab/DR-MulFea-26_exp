...
import sys, os, array, copy, random
from psychopy import visual, info, monitors, event, core
from psychopy.tools.monitorunittools import deg2pix, pix2cm, pix2deg
import numpy as np
#===============================================================================
# defining constants
#===============================================================================
...
class EyeLinkCoreGraphicsPsychopy(pylink.EyeLinkCustomDisplay):
    def __init__(self, tracker, display, displaySize, textStim = None):
        '''
        Initialize a Custom EyeLinkCoreGraphics for Psychopy
        tracker: the TRACKER() object
        display: the Psychopy display window
        displaySize: [width, height] in pix
        textStim: instance of textStim to use for presenting text during calibration
        '''
        pylink.EyeLinkCustomDisplay.__init__(self)
        self.display = display
        self.displaySize = displaySize
        self.setTracker(tracker)
        # NOTE: needed only for the dummy mode
        ...
        self.imtitle = visual.TextStim(self.display,
                                        pos = [0,
                                              int(self.displaySize[1] * .30)],
                                       height = self.text.height,
                                       wrapWidth = self.text.wrapWidth)
        self.img= visual.ImageStim(self.display)
        # pupil crosshair
        self.pxhair = visual.Line(self.display, units ='pix', lineWidth = 2)
        self.pyhair = visual.Line(self.display, units ='pix', lineWidth = 2)
        # pupil box
        self.pupilBox = visual.Rect(self.display, units='pix', lineWidth=2,
                                    # this is necessary as the rgb255 does not work
                                    lineColor = (1, 0, 0))
        # head crosshair
        self.headHair=[]
        for i in range(4):
    ...
    def setup_cal_display(self):
        ...
        '''
        welcomeText = ''.join(['Welcome to camera setup.\n'
                               'Press \"Return\" to display camera view.\n'
                               'Press \"C\" to calibrate eye-tracker.\n'
                               'Press \"V\" to validate calibration.\n'
                               'Press \"O\" to start experiment.'])
        self.text.setText(welcomeText)
    ...
    def erase_cal_target(self):
        '''
        Erase the calibration or validation target.
        '''
        self.display.flip()
    ...
    def draw_cal_target(self, x, y):
        ...
        self.tracker.target.setPos((x - 0.5 * self.displaySize[0],
                                    0.5 * self.displaySize[1] - y))
    ...
    # NOTE: this is needed only for dummy mode where the fixations are
    # generated as mouse clicks
    ...
    def setup_image_display(self, width, height):
        '''
        Assign the size of presentation area on display computer
        for camera feed
        '''
    ...
    def image_title(self, text):
        '''
        Set text for the camera setup display
        This shows which eye is being tracked and the detection thresholds
        '''
        # print(text)
    ...
    def draw_cross_hair_eyelinkCL(self):
        ...
        if(x[2] != 0x8000):     # pupil limits box
            self.pupilBox.setPos(((x[2] + x[3]) / 2.0,
                                  (y[2] + y[3]) / 2.0))
    ...
    def set_image_palette(self, r, g, b):
        '''
        Given a set of RGB colors, create a list of 24bit numbers representing the pallete.
...
class TrackerEyeLink():
    def __init__(self,
        ...
        '''
        # self.edfFileName = edfFileName
        # print(self.edfFileName)
        inf = info.RunTimeInfo("J","1",
                               win,
                               refreshTest = None,
        ...
        self.monitor = monitors.Monitor(self.monitorName)
        self.win = win

        # print("Connecting to eyetracker.")
        self.tracker = pylink.EyeLink()
        if target == None:
            self.tracker.target = visual.PatchStim(win,
                                                   tex = None,
                                                   mask = 'circle',
                                                   units='pix',
                                                   pos=(0,0),
        ...
        genv = EyeLinkCoreGraphicsPsychopy(self.tracker,
                                           win,
                                           self.screenSize,
                                           text)
        ...
        self.tracker.sendMessage("DISPLAY_COORDS  0 0 %d %d"
                                 %( tuple(self.screenSize)))
        self.tracker.sendCommand("sample_rate =  %d"
                                 %(sampleRate))
    ...
    def testCalibration(self):
        '''
        Check the drift and run calibration again, if necessary.
        '''
        while True:
            try:
                error = self.tracker.doDriftCorrect(int(self.screenSize[0]/2),
                                                    int(self.screenSize[1]/2),
                                                    1,
                                                    1)
    ...
    def beginRecording(self, win,
                       testCalibration = True,
                       autoDrift = False,
                       edfFileName = 'TEMP.EDF',):
        '''
        Prepare eye tracker to start recorting
        '''
        if testCalibration:
            self.testCalibration()
        self.edfFileName = edfFileName
        self.tracker.doSetup = False
        self.tracker.openDataFile('TEMP.EDF')
        self.tracker.sendMessage("PIXEL_SIZE deg:%.3f cm:%.3f"
                                 %(tuple([self._pix2(1, unit = 'deg'),
                                          self._pix2(1, unit = 'cm')])))
        ...
        if self.eye_used == RIGHT_EYE:
            self.tracker.sendMessage("PRETRIAL_EYE-USED_1-RIGHT")
        elif self.eye_used == LEFT_EYE :
            self.tracker.sendMessage("PRETRIAL_EYE-USED_0-LEFT")
        elif self.eye_used == BINOCULAR:
            self.tracker.sendMessage("PRETRIAL_EYE-USED_2-BINOCULAR")
        else:
        ...
        if autoDrift:
            ...
            while i < 10 and not (leftFinished and rightFinished):
                ...
                if sampleType == pylink.FIXUPDATE:
                    ...
                    if (((x - gazePos[0]) ** 2 + (y - gazePos[1]) **2)**0.5 < deg2pix(3,win.monitor) and
    ...
    def saveEyedata(self):
        ...
        self.tracker.receiveDataFile('TEMP.EDF',
                                     self.edfFileName)
    ...
    def getSample(self, unit = None):
        ...
        if self.eye_used == RIGHT_EYE and sample.isRightSample():
            eyePosPix = (np.array(sample.getRightEye().getGaze())
                         - 0.5*self.screenSize) * np.array([1,-1])
        elif self.eye_used == LEFT_EYE and sample.isLeftSample():
            eyePosPix = (np.array(sample.getLeftEye().getGaze())
                         - 0.5*self.screenSize) * np.array([1,-1])
        return eyePosPix

    def _pix2(self, pixels, unit = None):
        '''
        ...
        if unit == 'pix':
            return pixels
        if unit == 'cm':
            return pix2cm(pixels, self.monitor)
        if unit == 'deg':
            return pix2deg(pixels, self.monitor)
...
if __name__ == "__main__":
    ...
    win = visual.Window(size = (900,700), fullscr=True , allowGUI=False,
                        ...
                        winType = 'pyglet', monitor='ASUS',colorSpace='rgb255')
    ...
    eyeTracker = TrackerEyeLink(win, doSetup=True)
