...
def runTrial(win,
    ...
    feedbackFrames = 0 if np.isnan(feedbackDuration) else feedbackDuration
    simulatedResponse, simulatedResponseTime = simulateBehaviour(trialDuration
                                                                 - feedbackFrames,
                                                                 simulate)
    ...
    while True:
        ...
        if trialFrame == signalStart:
            ...
            # select motion directions so that the circle direction is
            # the average of the two ring directions
            dirL, dirR = wrapTopi(targetAngle
    ...
    return (dirsLeft, dirsRight, targetAngle, responseAngle, responseTime, flipTimes)
